...
const SatelliteTracker = ({ satellite_name, globeOptions = {} }) => {
      ...
      if (satellite_name) {
        try {
          const response = await axios.get(
            `/api/fetch-sat?name=${satellite_name}`
          );
          const satelliteData = response.data;
          if (satelliteData && satelliteData.tle && satelliteData.tle[1] && satelliteData.tle[2]) {
            setTleData(satelliteData.tle);
          } else {
            console.error("TLE data is incomplete or missing.");
            setTleData(["", "", ""]); // Set to empty to avoid issues
          }
        } catch (error) {
  ...
  useEffect(() => {
    if (tleData[1] && tleData[2]) {
      ...
      console.error("Skipping satellite tracking due to missing TLE data.");
    }
  }, [tleData]);
  ...
  return (
    <figure>
      <Globe
        ref={globeEl}
        globeImageUrl={
          globeOptions.globeImageUrl ||
          "//unpkg.com/three-globe/example/img/earth-night.jpg"
        }
        bumpImageUrl={
          globeOptions.bumpImageUrl ||
          "//unpkg.com/three-globe/example/img/earth-topology.png"
        }
        showAtmosphere={
          globeOptions.showAtmosphere !== undefined
            ? globeOptions.showAtmosphere
            : true
        }
